...
class JolpicaClient {
  ...
  async getSchedule()            { return this.get(JOLPICA_URLS.current,              this._getEffectiveTtlMs('SCHEDULE',  TTL.SCHEDULE));  }
  /** Convenience: fetch driver standings */
  async getDriverStandings()     { return this.get(JOLPICA_URLS.driverStandings,      this._getEffectiveTtlMs('STANDINGS', TTL.STANDINGS)); }
  /** Convenience: fetch constructor standings */
  async getConstructorStandings(){ return this.get(JOLPICA_URLS.constructorStandings, this._getEffectiveTtlMs('STANDINGS', TTL.STANDINGS)); }
  /** Convenience: fetch last race results */
  async getLastRaceResults()     { return this.get(JOLPICA_URLS.lastRaceResults,       this._getEffectiveTtlMs('LAST_RACE', TTL.LAST_RACE)); }
  ...
  /**
   * Return effective TTL in milliseconds for a given key.
   * Reads user-configured value (in hours) from Homey settings;
   * falls back to the compile-time default when not set.
   * @param {string} key        e.g. 'SCHEDULE'
   * @param {number} defaultMs  compile-time default TTL in ms
   */
  _getEffectiveTtlMs(key, defaultMs) {
    try {
      const stored = this._homey.settings.get('f1:ttl');
      if (stored && typeof stored[key] === 'number' && stored[key] > 0) {
        return stored[key] * 60 * 60 * 1000; // hours → ms
      }
